...
Time MelhorClassificadoContinente(Partida VetorPartidas[], int tam_part, Time VetorTimes[], int tam_times, int continente){
    ...
    if(qtd>1){
        maior=0;
        for(int i=0; i<tam_times; i++){
            if(VetorTimes[i].continente==continente && VetorTimes[i].saldo_gols>maior){
                maior=VetorTimes[i].saldo_gols;
                maiorContinenteI=i;
            }
        }
        ...
        if(qtd>1){
            maior=0;
            for(int i=0; i<tam_times; i++){
                if(VetorTimes[i].continente==continente && VetorTimes[i].gols_aFavor>maior){
                    maior=VetorTimes[i].gols_aFavor;
                    maiorContinenteI=i;
                }
            }
...
int main(){
    ...
    int primeiro=1, segundo=2, terceiro=3, quarto=4, quinto=5;
    america1=MelhorClassificadoContinente(VetorPartidas, tamPartidas, VetorTimes, tam, primeiro);
    europa2=MelhorClassificadoContinente(VetorPartidas, tamPartidas, VetorTimes, tam, segundo);
    africa3=MelhorClassificadoContinente(VetorPartidas, tamPartidas, VetorTimes, tam, terceiro);
    asia4=MelhorClassificadoContinente(VetorPartidas, tamPartidas, VetorTimes, tam, quarto);
    oceania5=MelhorClassificadoContinente(VetorPartidas, tamPartidas, VetorTimes, tam, quinto);
